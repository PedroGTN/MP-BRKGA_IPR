...
sol_file_list.remove(csv_file_name[:-9]+'.csv')

csv_file.write("num_nodes,instance_type,alpha,optimal_tsp_solution,best_fitness,opt_to_fit_dist,opt_to_fit_percent,current_iteration,last_update_iteration,current_time,last_update_time,stalled_iterations,largest_iteration_offset\n\n")
...
for inst_type in sol_file_list:
    ...
    for num in num_list:
        ...
        for alpha in alphas:
            ...
            csv_file.write(str(num) + ',' + inst_type + ',' + str(alpha) + ',')
            ...
            for inst in inst_list:
                ...
                for m in mode_list:
                    ...
                    runtime_list = os.listdir(runtime_folder)
            ...
            csv_file.write( ("{:.4f}".format(optimal_solution/10)) + ',' + ("{:.4f}".format(best_fitness/media)) + ',' + ("{:.4f}".format((best_fitness/media) - optimal_solution/10)) + ',' + ("{:.4f}".format(((best_fitness/media)/(optimal_solution/10))*100)) + '%,' + str(current_iteration/media) + ',' + str(last_update_iteration/media) + ',' + ("{:.4f}".format(current_time/media)) + ','+ ("{:.4f}".format(last_update_time/media)) + ',' + str(stalled_iterations/media) + ',' + str(largest_iteration_offset/media) + "\n")
            if(alpha == 3):
                csv_file.write('\n')
